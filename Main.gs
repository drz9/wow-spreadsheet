...
function main(raidID, guildName, guildRealmName) {
  ...
  for (var i = 0; i < raidbosses[0].length; i++) {
    ...
    lootDH.push(armor[3].concat(weapon[0], lootMisc))
    lootDK.push(armor[2].concat(weapon[1], lootMisc))
    lootDruid.push(armor[3].concat(weapon[2], lootMisc))
    lootHunter.push(armor[1].concat(weapon[3], lootMisc))
    lootMage.push(armor[0].concat(weapon[4], lootMisc))
    lootMonk.push(armor[3].concat(weapon[5], lootMisc))
    lootPaladin.push(armor[2].concat(weapon[6], lootMisc))
    lootPriest.push(armor[0].concat(weapon[7], lootMisc))
    lootRogue.push(armor[3].concat(weapon[8], lootMisc))
    lootShaman.push(armor[1].concat(weapon[9], lootMisc))
    lootWarlock.push(armor[0].concat(weapon[10], lootMisc))
    lootWarrior.push(armor[2].concat(weapon[11], lootMisc))
  }
